class Review:
	def __init__(self, reviewHtml):
		...
		self.recommendation = reviewHtml.select('span.user-post__author-recomendation > em')[0].text
		self.starCount = reviewHtml.select('span.user-post__score-count')[0].text
		self.reviewDate = reviewHtml.select('span.user-post__published > time[datetime]:nth-child(1)')[0]['datetime']
		self.purchaseDate = reviewHtml.select('span.user-post__published > time[datetime]:nth-child(2)')[0]['datetime']
		self.voteYesCount = reviewHtml.select('span[id^=votes-yes]')[0].text
		self.voteNoCount = reviewHtml.select('span[id^=votes-no]')[0].text
		self.content = reviewHtml.select('div.user-post__text')[0].text	
